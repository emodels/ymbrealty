...
function ShowOtherImages(){
   d = new Date();  
   var file_names_array = $('#image_others').val().split(',');
   
   $.each(file_names_array, function(index, filename) {
       if (filename != ''){
...
<div class="form">
    ...
    ?>
    <h1 style="font-size: 22px; padding: 10px"><b>Edit Property</b></h1>
    <?php
    $other_image_file_names = '';
    if (count($model->propertyImages) > 0){
        $other_image_array = array();
        foreach ($model->propertyImages as $value) {
            $other_image_array[] = $value->file_name;
        }
        $other_image_file_names = implode(",", $other_image_array);
    }
    ?>
    <?php echo $form->hiddenField($model, 'ref_no'); ?>
    <?php echo $form->hiddenField($model, 'image_main'); ?>
    <?php echo CHtml::hiddenField('image_others', $other_image_file_names); ?>
       ...
       <div class="row" style="padding-top: 10px">
        ...
        <div class="row" style="padding-top: 20px">
            ...
            <div class="column checkboxgroup">
                ...
                if (count($model->propertyCategories) > 0){
                    foreach ($model->propertyCategories as $value) {
                        $category_array[] = $value->category;
                    }
                }
                ?>
                <?php echo CHtml::checkBoxList('category', $category_array, CHtml::listData(Category::model()->findAll(), 'id', 'name'), array('separator'=>'', 'template'=>'<div><span>{input}</span><span>{label}</span></div>')); ?>
            </div>
            <div class="clearfix"></div>
        ...
        <div class="row" style="padding-top: 10px">
            ...
            <div class="column">
                <img id="img_main" src="<?php echo Yii::app()->baseUrl; ?>/property_images/<?php echo $model->image_main; ?>" class="upload_image" style="margin: 0px; display: <?php echo $model->image_main != '' ? 'block' : 'none'; ?>" /><br/>
        ...
        <div class="row" style="padding-top: 10px">
            ...
            <div class="column"><input type="submit" value="Update Property" class="button_orange" style="padding: 10px; font-size: 20px; cursor: pointer"/></div>
            <div class="clearfix"></div>
        </div>
    </div>
    <?php $this->endWidget(); ?>
